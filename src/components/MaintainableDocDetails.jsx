...
console.log(types)
types.map(type =>{
if (docData.insuranceTypes[type]=== .5 && !somewhat.includes(type)){
...
const update = () => {
 ...
 <div id= "col-cntnr">
  ...
  <ul>
  {suggested && suggested.map((type)=><><li>{type}
  </li>
  <Button onClick={()=>setUpdateVal({[type]:0})}>Set false</Button>
  <Button onClick={()=>setUpdateVal({[type]:.5})}>Set somewhat true</Button>
  </>
  )
  }
  </ul>
 ...
 <div>
  ...
  <ul>
  {somewhat && 
  somewhat.map((type)=><><li>{type}
 <Button onClick={()=>setUpdateVal({[type]:1})} >Set fully true</Button>
 <Button onClick={()=>setUpdateVal({[type]:0})}>Set false</Button>
  </li>
 </>)}
 </ul>
...
<ul>
<div id = "add">
{InsuranceList.map(item=> <>{!somewhat.includes(item)&& !suggested.includes(item)&&

<>
<li>{item}</li>
<Button onClick={()=>setUpdateVal({[item]:.5})}>Set this somewhat true</Button>
<Button onClick={()=>setUpdateVal({[item]:1})}>Set this fully true</Button>
</>
}</>)
}
</div>
